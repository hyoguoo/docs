...
인덱스를 통해 테이블의 레코드를 읽는 것은 바로 레코드를 읽는 것보다 높은 비용(4-5배)이 발생하기 때문에 너무 많은 데이터를 읽어야 한다면 인덱스를 사용하지 않는 것이 더 효율적이다.  
...
2. 인덱스 스캔(index scan): 위에서 탐색된 위치부터 필요한 만큼 인덱스를 차례대로 읽음
3. 읽어들인 인덱스 키와 레코드 주소를 이용해 실제 레코드를 한 건 한 건 읽어옴(랜덤 I/O 발생)

인덱스를 통해 데이터 레코드를 읽는 작업은 비용이 많이 드는 작업으로, 위에 언급했듯이 인덱스를 통해 읽어야 할 데이터 레코드가 20~25%를 넘어가면 인덱스를 사용하지 않고 직접 읽는 것이 더 효율적일 수 있다.
...
보통 인덱스의 크기는 테이블의 크기보다 작으므로 직접 테이블을 처음부터 끝까지 읽는 테이블 풀 스캔보다는 빠른 성능을 보여준다.  
...
위 인덱스를 효율적으로 사용하려면 WHERE 조건절에 gender 컬럼에 대한 비교 조건이 필수가 된다.(MySQL 8.0 버전 이전)
...
gender 컬럼에서 유니크한 값을 모두 조회하여 주어진 쿼리에 gender 컬럼의 조건을 추가해서 쿼리를 실행하면서 레인지 스캔을 수행한다.
