...
- 실제론 PK 없이 테이블이 생성되지만, 내부적으론 아래의 규칙에 따라 PK로 사용할 컬럼을 결정
    1. 지정된 PK를 사용
...
- 유사한것들을 묶는 것을 클러스터링이라고 하는데, 유사한 PK(Primary Key)끼리 물리적인 위치에 클러스터링하여 저장
- 결과적으로 PK 값의 순서대로 디스크에 저장되어 있어 PK를 이용한 레인지 스캔을 우선적으로 사용하게 됨
- 장점
    - PK만으로 데이터 레코드를 찾아올 수 있어 다른 인덱스를 통한 조회보다 빠름
    - 연속 조회 시에도 디스크에 PK 값의 순서대로 저장되어 랜덤 IO가 아닌 순차 IO로 읽어오기 때문에 성능 향상
        - 실행 계획에서 다른 보조 인덱스보다 PK를 이용한 레인지 스캔을 우선적으로 사용하게 됨
- 단점
    - 물리적인 위치를 순차적으로 유지해야 하기 때문에 레코드의 삽입/삭제를 하는 경우 성능 저하
