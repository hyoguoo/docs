...
![img.png](image/thread-lifecycle.png)
...
민액 다시 같은 값을 읽어오려고 하면 캐시에서 읽어오기 때문에 메모리의 값이 변경되어도 캐시에 저장된 값이 변경되지 않는다.
...
`volatile`를 변수에 적용하면 캐시가 아닌 메모리에서 직접 읽고 쓰도록 하여, 변수의 값이 변경되면 다른 쓰레드에서도 즉시 변경된 값을 읽을 수 있게 된다.
