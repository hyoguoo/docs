...
3. 해당 쓰레드가 임계 영역 내의 모든 코드를 수행하고 잠금을 반납
4. 다른 쓰레드가 잠금을 획득하여 임계 영역 내의 코드 수행
...
`volatile`를 변수에 적용하면 캐시가 아닌 항상 메모리에서 직접 읽고 쓰도록 하여, 변수의 값이 변경되면 다른 쓰레드에서도 즉시 변경된 값을 읽을 수 있게 된다.
