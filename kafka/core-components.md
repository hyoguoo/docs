...
- 키가 없는 경우 라운드 로빈 방식으로 파티션에 균등하게 분배하여 부하를 분산시킴
...
최신 카프카에서는 균등하게 분배하여 부하를 분산시키는 방법 대신, Sticky Partitioner + 배치 단위 전송 방식을 권장하고 있다.

- 여러 파티션이 존재할 때 순차적으로 메시지를 분산시키는 대신, 하나의 파티션에만 메시지를 전송
- 해당 파티션에 메시지가 가득 차거나 일정 시간이 지나면 다른 파티션으로 전환
- 메시지를 배치 단위로 묶어 전송하여 네트워크 오버헤드를 줄이고 처리량을 극대화
